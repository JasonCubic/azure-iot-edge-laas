...
const mssqlConfig = {
  user: _.get(process, 'env.MSSQL_USERNAME', 'SA'),
  password: _.get(process, 'env.MSSQL_PASSWORD', ''),
  server: _.get(process, 'env.MSSQL_SERVER', ''),
  database: _.get(process, 'env.MSSQL_DATABASE', ''),
...
async function upsertReader(readerObj) {
  ...
  try {
    ...
    console.error('getPool SQL error', err);
  }
